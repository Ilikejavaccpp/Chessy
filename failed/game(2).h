...
private:
  ...
  ChessUI::CHESSY_UI_MENU_MODE currentScreenMode =
      ChessUI::CHESSY_MODE_UNSELECTED;
  ChessMode::CHESSY_UTIL_MENU_MODE currentGameMode =
      ChessMode::CHESSY_MODE_NORMAL;
  ...
  void update() {
    ...
    //     }
    //     */
    //     ChessMenu::updateMenuInput(start_ui, width, height,
    //     currentScreenMode);
    //   }
    if (currentGameMode == ChessMode::CHESSY_MODE_NORMAL) {
      ChessMenu::UpdateMenuInput(start_ui, currentScreenMode, currentGameMode);
    } else if (currentGameMode == ChessMode::CHESSY_MODE_PLAYCF) {
      // Currently no logic for other stuff

      if (currentScreenMode ==
          ChessUI::CHESSY_UI_MENU_MODE::CHESSY_MODE_PRACTICE)
        ...
        ChessInput::ProcessDualInput(boardState, currentTurn, mouseInteraction,
                                     ...
                                     castling_rights, soundCheck, soundMove,
                                     soundCapture, soundCastle);
    }
  }
...
public:
  ChessApplication(int width, int height, const char *title) {
    ...
    // Initialize the start up menu
    ChessMenu::InitMenuLayout(start_ui);
  ...
  void run() {
    ...
    // Load the sounds
    // Will in the future make loading sounds a plugin via LoadSounds
    // method/util function which will load sounds from a Sound array called
    // `Sounds`
    soundCapture = LoadSound("assets/sounds/capture.wav");
    soundCastle = LoadSound("assets/sounds/castle.wav");
    soundCheck = LoadSound("assets/sounds/check.wav");
    soundMove = LoadSound("assets/sounds/move.wav");
    ...
    this->font_normal =
        ChessMenu::loadPieceGlyphs("assets/fonts/nffont_reg.ttf");
